...
class PaypalTransactionTransformer extends TransformerAbstract
{
    ...
    public function transform(PaypalTransaction $paypalTransaction): array
    {
        ...
        return [
            ...
            'datetime'                     => !empty($paypalTransaction->datetime) ? '<span class="x-has-time-converter">'.$paypalTransaction->datetime->format(
    ...
    public function renderActions(PaypalTransaction $paypalTransaction): string
    {
        return '
            ...
            '.($this->hasRefund($paypalTransaction) ?
                ActionWidget::renderRefundBtn(
                    $paypalTransaction->transaction_id,
                    $paypalTransaction->gross,
    ...
    /**
     *  Determines whether a PayPal transaction can be refunded.
     *
     * @throws \JsonException
     */
    public function hasRefund(PaypalTransaction $paypalTransaction): bool
    {
        if ($paypalTransaction->status !== "S" || $paypalTransaction->type !== "Express Checkout Payment") {
            return FALSE;
        }
        $paygate = Paygate::findOrFail($paypalTransaction->paygate_id);
        $paypalApi = new PayPalAPI($paygate);
        $transactionStatus = $paypalApi->getCapturedPaymentDetails(
            $paypalTransaction->transaction_id
        )['response']['status'];
        if ($transactionStatus === "PARTIALLY_REFUNDED" || $transactionStatus === "REFUNDED") {
            return FALSE;
        }
        return TRUE;
    }
