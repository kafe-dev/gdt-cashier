...
class DisputeCommand extends Command
{
            ...
            if (!empty($response['items'])) {
                foreach ($response['items'] as $item) {
                    try {
                        ...
                        echo 'update: ' . $item['dispute_id'] . '=>' . $item['status'] . PHP_EOL;
                    }
                }
            } else {
     ...
     * @throws \Exception
     */
    public function updateStatus(string $dispute_id, string $status)
    {
        ...
        if (!$dispute->save()) {
            throw new \Exception('Update status dispute failed');
        }
    }
}
